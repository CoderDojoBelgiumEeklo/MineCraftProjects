...
export class OTFAComponent implements AfterViewInit {
  ...
  ngAfterViewInit() {
    this.RefreshFile();
    this.pageSelectorService.change.subscribe(refresh => {
      this.RefreshFile();
    });
    this.translationhelperService.change.subscribe(refresh => {
      this.RefreshFile();
    });
  }
  ...
  OnTheflyComponent() {
    ...
    @Component({
      ...
      template:
        '<section><div id="CenterWrapper"><iframe [src]="getVideoUrl()" frameborder="0" ' +
    ...
    class YtPlayerComponent {
      ...
      getVideoUrl() {
        return this.sanitizer.bypassSecurityTrustResourceUrl(
          this.VideoLink.url.replace("watch?v=", "embed/")
        );
      }
    }
    ...
    @Component({
      selector: "pm-layout-top",
      templateUrl: "./layout-top/layout-top-component.html"
    })
    class LayoutTopComponent {
      @Input() partInfo: LayoutData;
    }

    @Component({
      selector: "pm-layout-bottom",
      templateUrl: "./layout-bottom/layout-bottom.component.html"
    })
    class LayoutBottomComponent {
      @Input() partInfo: LayoutData;
    }
    ...
    this._compiler
      .compileModuleAndAllComponentsAsync(this.tmpModule)
      .then(factories => {
  ...
  RefreshFile() {
    this.template = this.markdownService.getMarkdownFile().subscribe(
      ...
      error => {
        this.markdownService.handleError(error);
      }
    );
  }
}
